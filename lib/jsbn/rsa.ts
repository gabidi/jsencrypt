...
function pkcs1pad2(
  ...
  ba[--n] = 0;
  const rng = secureRandom ? secureRandom : new SecureRandom();
  const x = [];
  while (n > 2) {
    // random non-zero pad
    x[0] = 0;
...
export class RSAKey {
  private secureRandom:SecureRandom | null = null;
  constructor({ secureRandom }:{ secureRandom?:SecureRandom }) {
    this.n = null;
    this.e = 0;
    this.d = null;
    this.p = null;
  ...
  public generate(B:number, E:string) {
    const rng = this.secureRandom ? this.secureRandom : new SecureRandom();
    const qs = B >> 1;
    this.e = parseInt(E, 16);
